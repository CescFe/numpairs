...
data class GamePresentationState(
    ...
    fun dismissStripItemEntry(): GamePresentationState =
        when (modal) {
    ...
    fun dismissTileOperatorSelection(): GamePresentationState =
        when (modal) {
            is GameModalState.TileOperatorSelection -> copy(modal = null)
            else -> this
        }

    fun showTileOperandSelection(tileIndex: Int, slot: OperandSlot): GamePresentationState =
        copy(
    ...
    fun dismissTileOperandSelection(): GamePresentationState =
        when (modal) {
            is GameModalState.TileOperandSelection -> copy(modal = null)
            else -> this
        }

    fun clearModal(): GamePresentationState =
        if (modal == null) {
            this
        } else {
            copy(modal = null)
    ...
    fun isSuccessOverlayVisible(isPuzzleSolved: Boolean): Boolean =
        isPuzzleSolved && !isSuccessOverlayDismissed
}
